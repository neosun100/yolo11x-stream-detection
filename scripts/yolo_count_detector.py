...
OUTPUT_RTMP = "rtmp://rtmp/count/cam"
...
def main():
    ...
    # ??? ObjectCounter (??????)
    # region ?????: [(x1, y), (x2, y), (x2, y-20), (x1, y-20)]
    line_points = [(20, height//2), (width-20, height//2)]
    counter = ObjectCounter(model=model, region=line_points, show=False)
    ...
    try:
        ...
        while True:
            ...
            if now - last_log_time >= 5:
                ...
                print(f"[{int(elapsed)}s] ??? {frame_count} ??FPS: {fps_actual:.1f}, ??: {in_count}, ??: {out_count}")
                last_log_time = now
