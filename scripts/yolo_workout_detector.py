...
OUTPUT_RTMP = "rtmp://rtmp/workout/cam"
...
def main():
    ...
    # ??? AIGym (????????????? 6, 8, 10)
    gym = AIGym(model=model, kpts=[6, 8, 10], show=False)
    ...
    try:
        ...
        while True:
            ...
            if now - last_log_time >= 5:
                ...
                # ???????????
                count = getattr(gym, 'count', 0)
                print(f"[{int(elapsed)}s] ??? {frame_count} ??FPS: {fps_actual:.1f}, ????: {count}")
                last_log_time = now
