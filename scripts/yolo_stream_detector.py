...
# ??
MODEL_PATH = "yolo11x.pt"  # ?? YOLO11x ????

# ???????????????????? "cam"
STREAM_NAME = os.getenv("STREAM_NAME", "cam")
SOURCE_RTMP = f"rtmp://rtmp/live/{STREAM_NAME}"
OUTPUT_RTMP = f"rtmp://rtmp/detected/{STREAM_NAME}"
    ...
    try:
        ...
        while True:
            ...
            if now - last_log_time >= 5:
                ...
                print(f"[{int(elapsed)}s] ??? {frame_count} ?, FPS: {fps_actual:.1f}, ???: {detections} ???")
                last_log_time = now
