...
OUTPUT_RTMP = "rtmp://rtmp/speed/cam"
...
def main():
    ...
    # ??? SpeedEstimator (????????????)
    region = [(width//4, height//4), (3*width//4, height//4), (3*width//4, 3*height//4), (width//4, 3*height//4)]
    speed_estimator = SpeedEstimator(model=model, region=region, show=False)
    ...
    try:
        ...
        while True:
            ...
            # ?? SpeedEstimator ???
            results = speed_estimator.process(frame)
