...
OUTPUT_RTMP = "rtmp://rtmp/segment/cam"
    ...
    try:
        ...
        while True:
            ...
            if now - last_log_time >= 5:
                ...
                segments = len(results[0].boxes) if results[0].boxes is not None else 0
                print(f"[{int(elapsed)}s] ??? {frame_count} ??FPS: {fps_actual:.1f}, ???: {segments} ?????")
                last_log_time = now
