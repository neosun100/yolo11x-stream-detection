...
# ??
MODEL_PATH = "yolo11x-obb.pt"  # ???? OBB ????????  # ?? OBB ??
SOURCE_RTMP = "rtmp://rtmp/live/cam"
OUTPUT_RTMP = "rtmp://rtmp/obb/cam"
    ...
    try:
        ...
        while True:
            ...
            if now - last_log_time >= 5:
                ...
                obbs = len(results[0].obb) if results[0].obb is not None else 0
                print(f"[{int(elapsed)}s] ??? {frame_count} ??FPS: {fps_actual:.1f}, ???: {obbs} ? OBB ??")
                last_log_time = now
